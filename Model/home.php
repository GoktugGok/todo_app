...
if($process == 'list'){
    ...
    $q = $db->prepare("SELECT SUM(todos.progress) as toplam , COUNT(todos.status),status,
                        (SUM(todos.progress) / COUNT(todos.status)) as sonuc
                        FROM todos WHERE todos.user_id=?
                        GROUP BY todos.status ");
    $q->execute([get_session('id')]);
    $process = $q->fetchAll(PDO::FETCH_ASSOC);
    ...
    $q = $db->prepare("SELECT 
                        status,count(todos.id) as toplam,(SUM(todos.progress) / COUNT(todos.status)) as sonuc,
    ...
    if ($q->rowCount()) {
        return [
            'success' => true,
            'type' => 'success',
            'data' => array_merge(['istatistik' => $q->fetchAll(PDO::FETCH_ASSOC)], ['surec' => $todos],['progress' => $process])
        ];
    }else{
        return [
            'success' => true,
